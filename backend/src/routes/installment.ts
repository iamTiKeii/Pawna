...
import { generateFlatCollectionSchedule, InvalidLoanParamsError } from "../services/interest";
...
router.post("/", requirePermission(["CONTRACTS_MANAGE"]) as any, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Generate schedules
      const cycles = generateFlatCollectionSchedule(
        repayVal,
        duration,
        cDays,
        normalizedLoanDate
      ...
      await adjustDailyCash(
        ...
        employeeId,
        `Giải ngân hợp đồng trả góp ${contractCode}. Thực giao: ${netDisbursement} (Đưa khách: ${disbVal}, Trừ đóng trước kỳ 1: ${upfrontCash})`
      );
...
// 4. Pay Installment
router.post("/:id/pay", requirePermission(["CONTRACTS_OPERATE"]) as any, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await adjustDailyCash(
        ...
        "installment_pay",
        employeeId,
        `Thu góp định kỳ kỳ ${payment.cycle_number} HĐ trả góp ${payment.contract.contract_code}. Thực thu: ${payAmount}`
      );
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await adjustDailyCash(
        ...
        today,
        -refundAmount,
        "installment_cancel_pay",
        employeeId,
        `Hủy đóng góp kỳ ${payment.cycle_number} HĐ trả góp ${payment.contract.contract_code}. Trừ két: ${refundAmount}`
      );
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await adjustDailyCash(
        ...
        "installment_redeem",
        employeeId,
        `Tất toán sớm HĐ trả góp ${contract.contract_code}. Thu quỹ: ${totalAmount} (Dư góp còn lại: ${outstandingAmount}, Nợ cũ: ${outstandingDebt}, Khác: ${otherVal})`
      );
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await adjustDailyCash(
        ...
        today,
        -refundAmount,
        "installment_redeem_cancel",
        employeeId,
        `Hủy tất toán HĐ trả góp ${contract.contract_code}. Trừ két: ${refundAmount}`
      );
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await adjustDailyCash(
        ...
        today,
        value,
        "installment_debt_payment",
        employeeId,
        `Thu nợ cũ HĐ trả góp ${contract.contract_code}. Số tiền: ${value}`
      );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (dTx.type === "record_debt") {
        ...
        await adjustDailyCash(
          ...
          today,
          -amount,
          "installment_debt_revert",
          employeeId,
          `Hủy thu nợ cũ HĐ trả góp ${contract.contract_code}. Trừ két: ${amount}`
        );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (diff !== 0) {
        await adjustDailyCash(
          ...
          "contract_edit",
          employeeId,
          `Điều chỉnh vốn giải ngân HĐ trả góp ${contract.contract_code} do sửa thông số. Chênh lệch: ${-diff}`
        );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (netCashFlow !== 0) {
        await adjustDailyCash(
          ...
          "contract_deleted",
          employeeId,
          `Khấu trừ/Hoàn trả quỹ két do xóa hợp đồng trả góp ${contract.contract_code}. Lượng hoàn két: ${-netCashFlow}`
        );
      }
