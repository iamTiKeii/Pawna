...
async function recalculateUnsecuredSchedule(tx, contractId) {
    ...
    const newCycles = (0, interest_1.generateInterestSchedule)(Number(contract.initial_loan_amount), Number(contract.interest_rate), remainingDays, contract.period_value, contract.interest_type.code, startDate, false);
...
function calculateAccruedInterest(contract) {
    ...
    const dailyRate = (0, pawn_1.calculateDailyInterestRate)(principal, rate, pValue, interestTypeCode);
...
router.post("/", (0, permission_1.requirePermission)(["CONTRACTS_MANAGE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, storeId, normalizedLoanDate, -netDisbursement, "unsecured_disbursement", employeeId, `Giải ngân hợp đồng tín chấp ${contractCode}. Thực giao: ${netDisbursement} (Gốc: ${principal}, Trừ lãi trước: ${upfrontInterest})`);
            // Ledger log
...
router.post("/:id/extend", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            const newCycles = (0, interest_1.generateInterestSchedule)(Number(contract.initial_loan_amount), Number(contract.interest_rate), days, contract.period_value, contract.interest_type.code, oldMaturityDate, false);
