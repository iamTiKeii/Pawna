...
async function recalculatePawnSchedule(tx, contractId) {
    ...
    if (lastPaidCycle) {
        ...
        const daysUsed = Math.round((startDate.getTime() - new Date(contract.loan_date).getTime()) / (1000 * 60 * 60 * 24));
    ...
    const newCycles = (0, interest_1.generateInterestSchedule)(Number(contract.loan_amount), Number(contract.interest_rate), remainingDays, contract.period_value, contract.interest_type.code, startDate, false // upfront is only for the very first cycle of the contract, not during midway recalculations
    );
...
function calculateAccruedInterest(contract) {
    ...
    if (!interestTypeCode)
        return 0; // HĐ mồ côi (interest_type bị xóa) → không tính lãi
...
router.post("/", (0, permission_1.requirePermission)(["CONTRACTS_MANAGE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, storeId, normalizedLoanDate, -netDisbursement, "pawn_disbursement", employeeId, `Giải ngân hợp đồng cầm đồ ${contractCode}. Thực giao khách: ${netDisbursement} (Gốc: ${principal}, Trừ lãi trước: ${upfrontInterest})`);
            // Create ledger log
...
// 4. Pay Interest
router.post("/:id/pay-interest", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            const updatedPayment = await tx.pawnInterestPayment.update({
                ...
                data: {
                    ...
                    paid_date: (0, cash_1.normalizeToMidnight)(today),
                },
            });
            // Update cash fund (+ payAmount)
            await (0, cash_1.adjustDailyCash)(tx, payment.contract.branch_id, today, payAmount, "pawn_interest_pay", employeeId, `Thu lãi kỳ ${payment.cycle_number} HĐ ${payment.contract.contract_code}. Thực thu: ${payAmount}`);
            // Save to ledger
            await tx.pawnTransactionLedger.create({
                data: {
                    contract_id: contractId,
...
const handleCancelInterest = async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, payment.contract.branch_id, today, -refundAmount, "pawn_interest_cancel", employeeId, `Hủy thu lãi kỳ ${payment.cycle_number} HĐ ${payment.contract.contract_code}. Trừ két: ${refundAmount}`);
    ...
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await tx.pawnPrincipalTransaction.create({
                data: {
                    contract_id: contractId,
                    transaction_date: (0, cash_1.normalizeToMidnight)(date),
                    type: "pay_down",
            ...
            await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, date, paydownAmount, "pawn_principal_paydown", employeeId, `Khách đóng bớt gốc HĐ ${contract.contract_code}. Nhận: ${paydownAmount}`);
            // 4. Ledger
            await tx.pawnTransactionLedger.create({
                data: {
                    contract_id: contractId,
...
router.post("/:id/borrow-more", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        const contractId = req.params.id;
        const employeeId = req.user.id;
        const { amount, notes, transactionDate } = req.body;
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, date, -borrowAmount, "pawn_principal_borrow_more", employeeId, `Khách vay thêm gốc HĐ ${contract.contract_code}. Giải ngân: ${borrowAmount}`);
            // 4. Ledger
            await tx.pawnTransactionLedger.create({
                data: {
                    contract_id: contractId,
...
router.delete("/:id/principal-transaction/:txId", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        const contractId = req.params.id;
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            if (pTx.type === "pay_down") {
                ...
                await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, today, -amount, "pawn_principal_revert", employeeId, `Hủy thu nợ gốc HĐ ${contract.contract_code}. Trừ két: ${amount}`);
                await tx.pawnTransactionLedger.create({
                    data: {
                        contract_id: contractId,
                        employee_id: employeeId,
                ...
                await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, today, amount, "pawn_principal_revert", employeeId, `Hủy giải ngân thêm nợ gốc HĐ ${contract.contract_code}. Hoàn lại két: ${amount}`);
                await tx.pawnTransactionLedger.create({
                    data: {
                        contract_id: contractId,
                        employee_id: employeeId,
...
router.post("/:id/extend", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        const contractId = req.params.id;
        const employeeId = req.user.id;
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            const extension = await tx.pawnContractExtension.create({
                data: {
                    contract_id: contractId,
                    from_date: (0, cash_1.normalizeToMidnight)(oldMaturityDate),
                    to_date: (0, cash_1.normalizeToMidnight)(newMaturityDate),
                    extension_days: days,
            ...
            const newCycles = (0, interest_1.generateInterestSchedule)(Number(contract.loan_amount), Number(contract.interest_rate), days, contract.period_value, contract.interest_type.code, oldMaturityDate, false);
...
// 10. Cancel Extension
router.delete("/:id/extend/:extendId", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        const contractId = req.params.id;
    ...
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await tx.pawnInterestPayment.updateMany({
                where: {
                    contract_id: contractId,
                    to_date: { lte: (0, cash_1.normalizeToMidnight)(rDate) },
                    is_paid: false,
                },
                data: {
                    is_paid: true,
            ...
            await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, rDate, totalRedeem, "pawn_redeem", employeeId, `Chuộc đồ tất toán HĐ ${contract.contract_code}. Thu quỹ: ${totalRedeem} (Gốc: ${principal}, Nợ cũ: ${outstandingDebt}, Lãi tích lũy: ${interestAmount}, Khác: ${otherVal})`);
            // Ledger log
            await tx.pawnTransactionLedger.create({
                data: {
                    contract_id: contractId,
...
router.post("/:id/cancel-redeem", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, today, -refundAmount, "pawn_redeem_cancel", employeeId, `Hủy chuộc đồ tất toán HĐ ${contract.contract_code}. Trừ két hoàn khách: ${refundAmount}`);
            // Restore status
            await tx.pawnContract.update({
                where: { id: contractId },
                data: { status: "active" },
...
router.post("/:id/pay-debt", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, today, value, "pawn_debt_payment", employeeId, `Thu tiền trả nợ HĐ ${contract.contract_code}. Số tiền: ${value}`);
            // 4. Ledger
            await tx.pawnTransactionLedger.create({
                data: {
                    contract_id: contractId,
    ...
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            if (dTx.type === "record_debt") {
                ...
                await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, today, -amount, "pawn_debt_revert", employeeId, `Hủy thu nợ cũ HĐ ${contract.contract_code}. Trừ két: ${amount}`);
                await tx.pawnTransactionLedger.create({
                    data: {
                        contract_id: contractId,
                        employee_id: employeeId,
...
router.put("/:id", (0, permission_1.requirePermission)(["CONTRACTS_MANAGE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            if (diff !== 0) {
                await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, new Date(), -diff, // If newNet is higher, cash goes out (-diff). If lower, cash comes in (+diff).
                "contract_edit", employeeId, `Điều chỉnh vốn giải ngân HĐ ${contract.contract_code} do sửa thông số. Chênh lệch: ${-diff}`);
...
router.delete("/:id", (0, permission_1.requirePermission)(["CONTRACTS_MANAGE"]), async (req, res) => {
    try {
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            if (netCashFlow !== 0) {
                await (0, cash_1.adjustDailyCash)(tx, contract.branch_id, new Date(), -netCashFlow, "contract_deleted", employeeId, `Khấu trừ/Hoàn trả quỹ két do xóa hợp đồng cầm đồ ${contract.contract_code}. Lượng hoàn két: ${-netCashFlow}`);
            }
...
router.post("/:id/liquidate", (0, permission_1.requirePermission)(["CONTRACTS_OPERATE"]), async (req, res) => {
    try {
        const contractId = req.params.id;
        const employeeId = req.user.id;
        const storeId = req.user.branch_id;
        ...
        const result = await db_1.prisma.$transaction(async (tx) => {
            ...
            const voucherCode = await (0, codeGen_1.generateVoucherCode)(tx, "receipt");
            ...
            await (0, cash_1.adjustDailyCash)(tx, storeId, new Date(), price, "pawn_liquidation", employeeId, `Thu tiền thanh lý tài sản hợp đồng ${contract.contract_code}. Số tiền: ${price}`);
