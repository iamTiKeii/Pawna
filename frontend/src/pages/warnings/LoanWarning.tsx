...
export const LoanWarning: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`/api/warnings/loan?search=${search}`);
      setList(res.data);
    } catch (err: any) {
      setList([]);
    } finally {
    ...
    <div className="space-y-6 text-slate-800 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-800 mt-2">Hợp Đồng Tín Chấp Chậm Đóng Lãi</h1>
          <p className="text-slate-500 text-xs mt-0.5">Danh sách các hợp đồng cho vay tín chấp quá hạn đóng lãi định kỳ hoặc cần xử lý gấp.</p>
        </div>
        <button 
          onClick={fetchData}
              ...
              {list.length === 0 ? (
                ...
                list.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 border-b border-slate-100">
                    ...
                    <td className="px-4 py-3 truncate max-w-[150px]">{item.customer?.address}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <a href={`/contracts/unsecured/${item.id}`} className="btn btn-primary btn-xs text-white rounded-lg">Chi tiết</a>
                    </td>
                  </tr>
                ))
              )}
