...
export const SettingsPage: React.FC = () => {
  ...
  const [capitalInterestPeriod, setCapitalInterestPeriod] = useState("1");
  ...
  const fetchSystemSettings = async () => {
    try {
      ...
      const storedCapDays = Number(res.data.capital_interest_period_days) || 30;
      const displayCapPeriod = convertDaysToDisplayUnit(storedCapDays, "monthly");
      setCapitalInterestPeriod(String(displayCapPeriod));
  ...
  const handleSaveSystem = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const capPeriodInDays = convertDurationToDays(capitalInterestPeriod, "monthly");
      ...
      await axios.put("/api/settings", {
        ...
        capital_interest_period_days: capPeriodInDays,
    ...
    <div className="space-y-6 text-slate-800 animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 border-b border-slate-200/80 pb-2">
        ...
        >
          ...
          <span>Cấu hình chung & Ngân hàng</span>
        </button>
        ...
        >
          ...
          <span>Cấu hình Mẫu in Hợp đồng & Chứng từ</span>
        </button>
      </div>
        ...
        <form onSubmit={handleSaveSystem} className="space-y-6">
          ...
          {activeTab === "general" && (
            <>
              ...
              {/* Section: Capital Contract Default Settings */}
              <div className="border-t pt-6">
                <h3 className="font-bold text-base text-slate-800 border-b pb-2 mb-4 flex items-center gap-1.5">
                  <Printer className="w-4 h-4 text-amber-500" />
                  <span>Cấu hình Hợp đồng Góp vốn / Nguồn vốn</span>
                </h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="label text-slate-600 text-xs font-bold py-1">Kỳ trả lãi mặc định (Tháng)</label>
                    <div className="relative">
                      <input
                        type="number"
                        value={capitalInterestPeriod}
                        onChange={(e) => setCapitalInterestPeriod(e.target.value)}
                        placeholder="1"
                        className="input input-bordered input-sm w-full bg-white border-slate-200 focus:outline-none focus:border-amber-500 text-slate-800 rounded-lg text-xs pr-16"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">tháng</span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-1">Tự động quy đổi qua convertDurationToDays (1 tháng = 30 ngày) khi lưu cấu hình.</p>
                  </div>
                </div>
              </div>
            </>
          ...
          {/* TAB 2: CẤU HÌNH MẪU IN HỢP ĐỒNG & CHỨNG TỪ */}
          {activeTab === "templates" && (
            <div>
              <h3 className="font-bold text-base text-slate-800 border-b pb-2 mb-4 flex items-center gap-1.5">
                <Printer className="w-4 h-4 text-amber-500" />
              ...
              <p className="text-xs text-slate-500 mb-6">Chọn mẫu in mặc định cho từng loại hợp đồng và chứng từ phiếu thu/chi.</p>
          ...
          <div className="border-t pt-4 flex justify-end">
            <button
              type="submit"
              disabled={loading}
              className="btn btn-primary text-slate-950 bg-amber-500 hover:bg-amber-600 border-none btn-sm gap-1.5 rounded-xl font-bold px-6 shadow-sm shadow-amber-500/20 cursor-pointer"
            >
